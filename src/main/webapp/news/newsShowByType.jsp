 ...
 <head>
 	...
 	<script type="text/javascript"> 
 		function getOnePage(type){
    	  	...
    	  	//提交
    	  	//知道应该跳转的页数之后，改变name为page的input标签的value值为页数，然后提交，相当于请求NewsServlet
    	  	document.getElementById('myform').submit();
      	}
	</script>  
  </head>
  
  <body><div class="center" style="width:810px"> 
  	<form action="/news/servlet/NewsServlet?type1=showNewsByNewsType&newsType=${requestScope.newsType}" id="myform" method="post">
  	 
  	  <div class="newsShowByType_frame center" id="frameDiv">
  	  
  	  	<div class="newsShowByType_left" id="leftDiv">
  	  		<ul style="list-style-type: none;">
  	  		<!-- 如果新闻类型是“all”，即已经是显示全部了，那么“全部”就不是一个可以点击的超链接 ，其他新闻类型同理-->
				<c:choose>
				    ...
				    <c:otherwise>
						<li><a href="/news/servlet/NewsServlet?type1=showNewsByNewsType&newsType=all&page=1&pageSize=5">
  	  					全部</a></li>	
				    </c:otherwise>			        
				</c:choose>	  	  			
	 			...
	 			<c:forEach items="${requestScope.newsTypes}"  var="newsType1">	  	 			
					<c:choose>
					    ...
					    <c:otherwise>
					    	<li><a href="/news/servlet/NewsServlet?type1=showNewsByNewsType&newsType=${newsType1.name}&page=1&pageSize=5">
					    		${newsType1.name}</a></li>
					    </c:otherwise>				        
					</c:choose>				
				</c:forEach> 	  		
			...
			<div>
				<ul>
					<c:forEach items="${requestScope.newses}"  var="news"> 
						<li><a href="/news/servlet/NewsServlet?type1=showANews&newsId=${news.newsId}&page=1&pageSize=2" target="_blank">${news.caption}</a></li>
					</c:forEach>
				</ul>
			</div>	
		</div>
